...
-- TEMP FOR TESTING (2026-08-31): 300 -> 30 сек, вернуть после проверки соло-босса
local DUEL_INTERVAL = 30
...
function DuelController:EndDuel(i_winnerTeam)
	...
	-- solo mode: despawn the duel boss if it is still alive (timeout / wipe)
	if SoloDuel and SoloDuel:IsActive() then
		SoloDuel:OnDuelEnded()
	end
	...
	-- AUGMENTS: duel grants a rare augment to the winner team, common to the loser team
	if i_winnerTeam ~= DuelLibrary.DRAW_TEAM and Augments and Augments.QueueForTeam then
		local loserTeam = i_winnerTeam == DOTA_TEAM_GOODGUYS and DOTA_TEAM_BADGUYS or DOTA_TEAM_GOODGUYS
		Augments:QueueForTeam(i_winnerTeam, AUGMENT_TIER.RARE)
		Augments:QueueForTeam(loserTeam, AUGMENT_TIER.COMMON)
	end
...
function DuelController:_StartDuel()
	...
	-- SOLO MODE: the whole radiant team fights a duel boss instead of a PvP duel
	if SoloMode and SoloMode:IsActive() then
		print("DuelController: solo mode, starting solo duel")
		local ok = SoloDuel:StartSoloDuel()
		if ok then
