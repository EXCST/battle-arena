...
function Util:DisableSpellAmp(caster, damage)
	...
	if caster:IsRealHero() then
		damage_int_pct_add = caster:GetIntellect(true)
...
function Util:DealDamageFromStats(victim, attacker, damage_type, str_damage, agi_damage, int_damage, spell_amp)
	...
	local damage = attacker:GetStrength(true)*str_damage + attacker:GetAgility(true)*agi_damage + attacker:GetIntellect(true)*int_damage
...
function Util:_DealDamage(victim, attacker, damage_type, damage_const, damage_pct, from_max_health)
	...
	if attacker:IsRealHero() then
		damage_int_pct_add = attacker:GetIntellect(true)
		damage_int_pct_add = damage_int_pct_add / 16 / 100 + 1
	end 
