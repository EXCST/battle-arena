...
function ChangeHero(player, oldHero, newHeroName, successCallback, failedCallback, checkFunction, repickItemName)
	...
	PrecacheUnitByNameAsync(newHeroName, function()
		...
		-- The engine applies the player's loadout (cosmetic sets) right after
		-- ReplaceHeroWith, but in custom games those item models are NOT resident
		-- (they were never precached) -> "nonresident model" ERROR / broken visuals.
		-- Precache each wearable model and force a reload on the wearable entity.
		for _, child in pairs(hero:GetChildren()) do
			if child:GetClassname() == "dota_item_wearable" then
				local model = child:GetModelName()

				if model and model ~= "" then
					PrecacheResource("model", model, nil)
					child:SetModel(model)
				end
			end
		end

		-- Remove default inventory wearables before they render (only for gods that
		-- rely on runtime cosmetics via modifier_doom_litany_cosmetics)
		if newHeroName == "npc_dota_hero_doom_bringer" or newHeroName == "npc_hero_satan" then
			for _, child in pairs(hero:GetChildren()) do
				if child:GetClassname() == "dota_item_wearable" then
					child:RemoveSelf()
				end
		...
		-- Apply Litany of the Damned on Doom/Satan via modifier
		if newHeroName == "npc_dota_hero_doom_bringer" or newHeroName == "npc_hero_satan" then
			hero:AddNewModifier(hero, nil, "modifier_doom_litany_cosmetics", {})
		end
