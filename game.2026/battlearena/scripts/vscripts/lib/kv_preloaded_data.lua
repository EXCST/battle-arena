...
function PreloadCache:_init()
	...
	if not self.hero_data then
		...
		for hero_name, hero_info in pairs(self.hero_data) do
			...
			-- self-override (npc_dota_hero_faceless_void points at itself) would delete the hero — skip
			if hero_override_name and hero_override_name ~= hero_name and already_hero_data and not already_hero_data['new_hero_name'] and not hero_info['IsCustom'] then
			...
			if hero_name == "npc_dota_hero_monkey_king" or hero_name == "npc_dota_hero_stegius" then
				print("[DIAG] " .. hero_name .. ": override_hero=" .. tostring(hero_override_name) .. ", Model=" .. tostring(hero_info['Model']) .. ", BaseClass=" .. tostring(hero_info['BaseClass']) .. ", HeroID=" .. tostring(hero_info['HeroID']) .. ", IsCustom=" .. tostring(hero_info['IsCustom']) .. ", Enabled=" .. tostring(hero_info['Enabled']) .. ", AttackCapabilities=" .. tostring(hero_info['AttackCapabilities']))
			end
		end
