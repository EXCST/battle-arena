...
local BOSS_FOR_REPICK = "target_mark_boss_omniknight"
...
function RepickMenu:CanPickNow()
	return (BossSpawner.death_count[BOSS_FOR_REPICK] or 0) > 0 
		and not BossSpawner.is_alive[BOSS_FOR_REPICK]
		and not BossSpawner.repick_used
end
...
function RepickMenu:PickHero(player, newHeroName)
	...
	local onSuccess = function()
		...
		BossSpawner.repick_used = true
		BossSpawner:_RemoveRepickZone(BOSS_FOR_REPICK)
		CustomGameEventManager:Send_ServerToAllClients("aa_repick_menu_close", {})
	end
	...
	ChangeHero(player, hero, newHeroName, onSuccess, onFailed, checkFunction, nil)
