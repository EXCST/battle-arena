...
-- безопасный traceback: в этой сборке debug = nil, из-за чего реальная
-- ошибка таймера теряется ("error in error handling")
...
function Timers:Think()
  ...
  if profileTime > Timers._timeLimit and nFuncs > 0 then
    ...
    for _, funcData in pairs(profileFunctions) do
      ...
      if funcObj ~= nil and debug ~= nil then
        local ok, funcInfo = pcall(debug.getinfo, funcObj)

        if ok and funcInfo then
  ...
  return TIMERS_THINK
    ...
    local m = type(msg) == "string" and msg or tostring(msg)
    return m .. " TRACEBACK: " .. SafeTraceback()
  end)
  if not ok then
    print("[DIAG] Timers:Think error: " .. tostring(err))
  end
