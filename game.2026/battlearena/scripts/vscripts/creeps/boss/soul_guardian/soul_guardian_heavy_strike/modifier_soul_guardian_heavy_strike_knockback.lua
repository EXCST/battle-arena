...
function mod:OnCreated(kv)
	...
	self.point = point
	self.duration = tonumber(kv.dur) or 0.3
...
function mod:UpdateHorizontalMotion(me, dt)
	if not IsServer() then return end
	if self and self:GetParent() and IsValidEntity(self:GetParent()) and self:GetParent():IsAlive() then
		local step = self.dir * (self.step * dt)
		local origin = self:GetParent():GetAbsOrigin()
		self:GetParent():SetAbsOrigin(origin + step)
	else
		if self and self:GetParent() and IsValidEntity(self:GetParent()) then
