...
function mod:OnCreated(kv)
	...
	self.aspeedPerHP = AbilityKV:Get(ability, "aspeed_per_hp_pct")
	if not self.aspeedPerHP or self.aspeedPerHP <= 0 then self.aspeedPerHP = 7 end
end
...
function mod:OnTakeDamage( params )
	...
	-- Marshall to AI info about taking some damage (если у ИИ есть такой метод)
	if parent.ai and parent.ai.OnTakeDamage then
