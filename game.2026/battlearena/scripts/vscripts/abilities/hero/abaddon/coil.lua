...
function AbaddonCoilStart( event )
	...
	local damage 			= ability:GetSpecialValueFor( "target_damage" ) + Util:DisableSpellAmp(caster, caster:GetStrength(true)*damage_pct)
	local heal 				= ability:GetSpecialValueFor( "heal_amount" ) + caster:GetStrength(true)*damage_pct
