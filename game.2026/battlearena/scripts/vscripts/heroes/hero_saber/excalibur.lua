...
function saber_excalibur:GetCastRange()
	...
	if not caster or caster:IsNull() then return AbilityKV:Get(self, "cast_range") end
	return AbilityKV:Get(self, caster:HasScepter() and "cast_range_scepter" or "cast_range")
end
if IsServer() then
	...
	function saber_excalibur:OnChannelFinish(bInterrupted)
		if not bInterrupted then
			local mana = self:GetCaster():GetMana() * AbilityKV:Get(self, "mana_pct") * 0.01
			local caster = self:GetCaster()
			if caster:GetMana() >= mana then
				caster:SpendMana(mana, self)
				local width = AbilityKV:Get(self, "width")
				local damage = mana * AbilityKV:Get(self, caster:HasScepter() and "damage_per_mana_scepter" or "damage_per_mana")
				...
				caster:StartGesture(ACT_DOTA_CHANNEL_END_ABILITY_5)
				for i = 1, self:GetCaster():HasScepter() and 3 or 1 do
					...
					Timers:CreateTimer(AbilityKV:Get(self, "delay"), function()
				...
				SendErrorMessage(caster:GetPlayerID(), "#dota_hud_error_not_enough_mana")
			end
		end
	end
else
	function saber_excalibur:GetManaCost()
		...
		return caster:GetMana() * AbilityKV:Get(self, "mana_pct") * 0.01
	end
end
