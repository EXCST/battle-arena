...
function CheckEnemies( keys )
	...
	-- GetLevelSpecialValueFor в этой сборке возвращает 0 (регрессия движка) —
	-- читаем KV напрямую через AbilityKV:Get
	local radius = AbilityKV:Get(ability, "radius")
	if not radius or radius <= 0 then radius = 1000 end

	-- единый модификатор стат-талантов (паттерн stegius): вешается один раз
	if not caster:HasModifier("modifier_arthas_talent_stats") then
		caster:AddNewModifier(caster, ability, "modifier_arthas_talent_stats", {})
	end
