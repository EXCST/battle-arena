...
function ability:GetCooldown(iLevel)
	...
	if caster and not caster:IsNull() then
		local talent = caster:FindAbilityByName(talentNameCD)
...
function ability:GetBehavior()
	local caster = self:GetCaster()
	if caster and not caster:IsNull() then
		local talent = caster:FindAbilityByName(talentNameAOE)
		if talent and talent:GetLevel() > 0 then
...
function ability:OnSpellStart( ... )
	...
	local talentAOE = caster:FindAbilityByName(talentNameAOE)
	local isAOE = talentAOE and talentAOE:GetLevel() > 0
