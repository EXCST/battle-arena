...
function item_devour_helm:CastFilterResultTarget( hTarget )
	...
	if hTarget:HasModifier("modifier_scaling_camp_boss") then
		return UF_FAIL_CUSTOM
	end
...
function item_devour_helm:GetCustomCastErrorTarget( hTarget )
	...
	if not hTarget:IsCreep() and not hTarget:IsHero() and not hTarget:IsConsideredHero() and not IsLoneDruidBear(hTarget)  then
		return "#dota_hud_error_cant_cast_on_other"
	end 
...
function item_devour_helm:OnSpellStart()
	...
	if self.target == nil then
		if original_target:HasModifier("modifier_scaling_camp_boss") then return end
