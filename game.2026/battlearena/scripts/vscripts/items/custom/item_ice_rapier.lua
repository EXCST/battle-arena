...
function item_ice_rapier:OnSpellStart()
	if IsServer() then
		...
		if caster:IsRealHero() then
			radius = radius + caster:GetStrength(true) * self:GetSpecialValueFor("radius_per_str")
			if radius > radiusCap then radius = radiusCap end
			duration = duration + caster:GetAgility(true) * self:GetSpecialValueFor("duration_per_agi")
	...
	if IsServer() then
		...
		if(self.parent and self.parent:IsNull() == false) then
			...
			if self.parent.GetStrength then
				iradius = self.parent:GetStrength(true) * self.ability:GetSpecialValueFor("radius_per_str")
			end
