-- Custom table replication mechanism, ported from wod and adapted for battlearena.
-- Differences from the original:
--   * RegisterLoadListener replaced by a player_connect_full hook wired in addon_game_mode.lua
--     (customers call CustomTables:OnRequestCustomTables({PlayerID = pid}) themselves)
--   * table.deep_equal replaced with battlearena's table.deep_compare (identical semantics)
...
function CustomTables:SendTableValueToAllClients(stringTableName, stringKeyName, script_tableValue)
    ...
    if last_value ~= nil and table.deep_compare(last_value, script_tableValue) then
        return false
    end
