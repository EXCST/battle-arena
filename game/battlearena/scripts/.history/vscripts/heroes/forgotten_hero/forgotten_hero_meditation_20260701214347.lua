...
function ability:OnSpellStart( kv )
	...
	-- Instant heal based on regen value and duration
	local regen = self:GetSpecialValueFor("hp_regen") or 0
	local dur = self:GetSpecialValueFor("duration") or 0
	local heal = regen * dur
	if heal > 0 then
		caster:Heal(heal, self)
	end

	-- Destroy old modifier if already exists (prevent stacking on recast)
	caster:RemoveModifierByName("modifier_forgotten_hero_meditation")
end
...
function ability:OnChannelFinish( bInterrupted )
	...
	if not bInterrupted and caster:HasTalent(talentName) then
		local duration = caster:GetTalentSpecialValueFor(talentName, "duration") or 0
	...
	-- Ensure animation stops after channel ends
