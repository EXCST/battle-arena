modifier_item_soul_collector_buff = modifier_item_soul_collector_buff or class({})
modifier_item_soul_collector_buff.sign = 1

function modifier_item_soul_collector_buff:IsPurgable() return false end
function modifier_item_soul_collector_buff:IsPurgeException() return true end
function modifier_item_soul_collector_buff:IsDebuff() return false end
function modifier_item_soul_collector_buff:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_item_soul_collector_buff:RemoveOnDeath() return true end
...
function modifier_item_soul_collector_buff:OnIntervalThink()
...
function modifier_item_soul_collector_buff:CheckState()
	return {
		[MODIFIER_STATE_TETHERED] = self.sign < 0,
		[MODIFIER_STATE_MUTED] = self.sign < 0,
	}
end
...
function modifier_item_soul_collector_buff:GetModifierMoveSpeedBonus_Percentage()
	return self.sign * self.movespeed_change_max * (self:GetElapsedTime() / self:GetDuration())
end

function modifier_item_soul_collector_buff:GetCustomLifestealAmplification()
	return self.sign * self.heal_change_max * (self:GetElapsedTime() / self:GetDuration())
end

function modifier_item_soul_collector_buff:GetModifierHealAmplify_PercentageSource()
	return self.sign * self.heal_change_max * (self:GetElapsedTime() / self:GetDuration())
end

function modifier_item_soul_collector_buff:GetModifierHealAmplify_PercentageTarget()
	return self.sign * self.heal_change_max * (self:GetElapsedTime() / self:GetDuration())
end

function modifier_item_soul_collector_buff:GetModifierHPRegenAmplify_Percentage()
	return self.sign * self.heal_change_max * (self:GetElapsedTime() / self:GetDuration())
end



modifier_item_soul_collector_debuff = modifier_item_soul_collector_debuff or class(modifier_item_soul_collector_buff)
modifier_item_soul_collector_debuff.sign = -1

function modifier_item_soul_collector_debuff:IsDebuff() return true end
