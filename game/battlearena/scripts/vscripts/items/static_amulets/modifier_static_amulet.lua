...
--------------------------------------------------------------------------------
function static_mod_class:OnCreated()
	...
	if IsServer() then
		if not self:GetCaster():HasModifier("modifier_static_damage") then
			self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_static_damage", {})
		end
	end
end

function static_mod_class:OnDestroy()
	...
	local callback = function()
		...
		if caster:HasItemInInventory('item_slice_amulet') or caster:HasItemInInventory('item_static_amulet') then
			return
		end
		if caster:HasModifier("modifier_static_damage") then
			caster:RemoveModifierByName("modifier_static_damage")
		end
	end
	
	Timers:CreateTimer(0, function() callback(caster) end)
end
-----------------------------------------------------------------------------
