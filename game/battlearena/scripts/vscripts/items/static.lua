...
local blocked_abilities = {
	...
	["tinker_rearm_lua"] = 1,
	["tinker_defense_matrix"] = 1,
	["tinker_keen_teleport"] = 1,
...
function DealDamageAroundCaster(caster, multiplier, damage_add, manacost, ability)
	...
	if caster:IsRealHero() then
		damage_int_pct_add = caster:GetPrimaryStatValue()
	...
	for _, x in pairs(units) do
		...
		local damage_illusion_pct = 35
        local damage_illusion = x:GetMaxHealth() / 100 * damage_illusion_pct
        ApplyDamage({ victim = x, attacker = caster, damage = damage_illusion, damage_type = DAMAGE_TYPE_PURE, ability = ability })
        end
	end
end
