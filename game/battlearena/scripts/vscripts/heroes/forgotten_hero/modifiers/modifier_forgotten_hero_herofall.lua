...
function mod:OnCreated( kv )
	...
	-- Fallback defaults if KV values are missing or zero
	if self.height == 0 then self.height = 250 end
	if self.speed == 0 then self.speed = 600 end
	if self.damage == 0 then self.damage = 125 end
	if self.stunDur == 0 then self.stunDur = 1 end
	...
	-- Fallback for AoE radius if KV is missing or zero
	if self.aoe == 0 then self.aoe = 300 end
