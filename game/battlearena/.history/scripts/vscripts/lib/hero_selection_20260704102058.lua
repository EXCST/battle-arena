...
function HeroSelection:_FinishSelection()
    ...
    -- НЕ вызываем GameRules:FinishCustomGameSetup() здесь:
    -- игра уже находится в фазе HERO_SELECTION, а не CUSTOM_GAME_SETUP.
    -- Этот вызов некорректен и может ломать переход состояний.

    -- Собираем список уже занятых героев (выбранных и забаненных),
    -- чтобы не назначить их повторно игрокам, не сделавшим выбор.
    local used = {}
    for _, info in pairs(self.state.picked) do
        used[info.hero] = true
    end
    for hero_name, _ in pairs(self.state.banned) do
        local base = HeroSelectionConfig:IsCustomHero(hero_name) and HeroSelectionConfig:GetBaseHero(hero_name) or hero_name
        used[base] = true
    end

    -- Формируем плоский список доступных обычных героев для случайного выбора.
    local available = {}
    if self.hero_pool and self.hero_pool.normal then
    ...
    local avail_idx = 1
    local function next_available_hero()
        while avail_idx <= #available do
            local h = available[avail_idx]
        ...
        -- Запасной вариант, если пул исчерпан.
    ...
    Timers:CreateTimer(0.5, function()
        ...
        for pid = 0, playerCount - 1 do
            ...
            if player then
                local info = self.state.picked[pid]
                local hero_to_assign
                if info then
                    hero_to_assign = info.hero
                else
                    -- Игрок не выбрал героя — назначаем случайного из доступных.
                    hero_to_assign = next_available_hero()
                    print("[HeroSelection] Player " .. pid .. " didn't pick, assigning random: " .. tostring(hero_to_assign))
                end

                local ok, err = pcall(function()
                    PlayerResource:ReplaceHeroWith(pid, hero_to_assign, 625, 0)
                end)
                if ok then
                    print("[HeroSelection] Assigned " .. hero_to_assign .. " to player " .. pid)
                else
