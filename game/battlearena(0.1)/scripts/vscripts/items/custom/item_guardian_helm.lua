...
function modifier_item_guardian_helm:GetModifierBonusHealth()
	if(self.parent.GetStrength) then
		return self:GetAbility():GetSpecialValueFor("str_health")*self.parent:GetStrength()
	end
	return 0
end
...
function modifier_item_guardian_helm:GetModifierConstantHealthRegen()
	if(self.parent.GetStrength) then
		return self:GetAbility():GetSpecialValueFor("str_regen")*self.parent:GetStrength()
	end
	return 0
end
