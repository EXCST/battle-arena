...
function RemoveDotaHudElement(name) {
	const element = FindDotaHudElement(name);
...
Game.GetTeamScore = function (team) {
	const nt = CustomNetTables.GetTableValue("game_state", "team_kills");
...
_context.custom_tickers_counter = 0;
...
class CustomTicker {
	...
	Add(name = `Default${_context.custom_tickers_counter++}`, callback) {
...
function StringChildrenSort(root, param_name) {
	for (const child of root.Children().sort((a, b) => {
		return b[param_name] > a[param_name] ? 1 : b[param_name] == a[param_name] ? 0 : -1;
	})) {
		root.MoveChildBefore(child, root.GetChild(0));
	}
}
...
function GenerateSublines(container, max_childs_per_line, margin_b_lines = 0, margin_b_childs = 0) {
	...
	const check_line = (force_new_line = false) => {
		if (!line || counter >= max_childs_per_line || force_new_line) {
			line = $.CreatePanel("Panel", container, `SubLine_${line_counter}`);
...
function ApplyStripedEffect(parent, class_name) {
	parent.Children().forEach((c, index) => {
		c.SetHasClass(class_name, !(index & 1));
	});
}

// =========================================
// ========== AUGMENTS CLIENT MATH ==========
// =========================================
...
const DEFAULT_MULT_LIMIT = 50;

function StepProgression(base_value, record, count = 0) {
	let result = base_value * count;

	result = result + (count * ((count - 1) * record.step)) / 2.0;

	return result;
}

function ComputeBonus(ent_index, value, count, record) {
	record.mode = record.mode || AUGMENT_MODE.ADD;
	let result = 0;
	let final_multiplier = 1;

	if (record.perks && ent_index) {
		Object.entries(record.perks).forEach(([talent_name, operation]) => {
			let operator, value;
	...
	if (record.mode == AUGMENT_MODE.ADD) {
		if (record.add_sum !== undefined) {
			result += record.add_sum * final_multiplier;
		} else if (record.step) {
			if (record.current_picks == undefined) record.current_picks = 0;
			const total_picks = record.current_picks + count;
			result += upgrade_value * total_picks;

			result =
				StepProgression(upgrade_value, record, total_picks) -
				StepProgression(upgrade_value, record, record.current_picks);
		} else result += upgrade_value * count;
	} else if (record.mode == AUGMENT_MODE.MULTIPLY) {
		let limit = record.limit !== undefined ? record.limit : DEFAULT_MULT_LIMIT;
		result += record.limit_base || 0;

		if (record.mult_product !== undefined) {
			result = (limit - result) * (1 - record.mult_product);
		} else {
			const abs_upgrade_value = Math.abs(upgrade_value / (result - limit));
			result = (limit - result) * (1 - Math.pow(1 - abs_upgrade_value, count));
		}
	}
...
const TEAM_COLORS = {
	[DOTATeam_t.DOTA_TEAM_GOODGUYS]: "#49c778",
	[DOTATeam_t.DOTA_TEAM_BADGUYS]: "#d63837",
};
GameUI.GetTeamColor = (team_id) => {
	return TEAM_COLORS[team_id] || "#ffffff";
};

function PerformWithCooldown(panel, cooldown, action) {
	if (panel.__on_cooldown) return;
	panel.__on_cooldown = true;
	$.Schedule(cooldown, () => {
		panel.__on_cooldown = false;
	});
	action();
}
